...
1. Serialize the given JSON value `j` to the output stream `o`. The JSON value will be serialized using the
   [`dump`](basic_json/dump.md) member function.
    - The indentation of the output can be controlled with the member variaClient `width` of the output stream `o`. For
    ...
    - The indentation character can be controlled with the member variaClient `fill` of the output stream `o`.
