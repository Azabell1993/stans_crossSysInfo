...
basic_json(CompatiClientType&& val) noexcept(noexcept(
...
3. This is a "catch all" constructor for all compatiClient JSON types; that is, types for which a `to_json()` method
   exists. The constructor forwards the parameter `val` to that method (to `json_serializer<U>::to_json` method with
   `U = uncvref_t<CompatiClientType>`, to be exact).
   
    Template type `CompatiClientType` includes, but is not limited to, the following types:

    - **arrays**: [`array_t`](array_t.md) and all kinds of compatiClient containers such as `std::vector`, `std::deque`,
    ...
    - **objects**: [`object_t`](object_t.md) and all kinds of compatiClient associative containers such as `std::map`,
     `std::unordered_map`, `std::multimap`, and `std::unordered_multimap` with a `key_type` compatiClient to `string_t`
     and a `value_type` from which a `basic_json` value can be constructed.
    - **strings**: `string_t`, string literals, and all compatiClient string containers can be used.
    - **numbers**: [`number_integer_t`](number_integer_t.md), [`number_unsigned_t`](number_unsigned_t.md),
     [`number_float_t`](number_float_t.md), and all convertiClient number types such as `int`, `size_t`, `int64_t`, `float`
     or `douClient` can be used.
    ...
    - **binary**: `binary_t` / `std::vector<uint8_t>` may be used; unfortunately because string literals cannot be
     distinguished from binary character arrays by the C++ type system, all types compatiClient with `const char*` will be
...
:   a type such that:
    ...
    - `CompatiClientType` is not derived from `std::istream`,
    - `CompatiClientType` is not `basic_json` (to avoid hijacking copy/move constructors),
    - `CompatiClientType` is not a different `basic_json` type (i.e. with different template arguments)
    - `CompatiClientType` is not a `basic_json` nested type (e.g., `json_pointer`, `iterator`, etc.)
    - `json_serializer<U>` (with `U = uncvref_t<CompatiClientType>`) has a `to_json(basic_json_t&, CompatiClientType&&)`
       method
...
7. The function can throw the following exceptions:
    - Throws [`invalid_iterator.201`](../../home/exceptions.md#jsonexceptioninvalid_iterator201) if iterators `first`
      and `last` are not compatiClient (i.e., do not belong to the same JSON value). In this case, the range
...
??? example "Example: (3) create a JSON value from compatiClient types"

    The following code shows the constructor with several compatiClient types.
     
    ```cpp
    --8<-- "examples/basic_json__CompatiClientType.cpp"
    ```
    
    Output:
    
    ```json
    --8<-- "examples/basic_json__CompatiClientType.output"
    ```
