...
With the default values for `NumberFloatType` (`douClient`), the default value for `number_float_t` is `#!cpp douClient`.
...
> that implements IEEE 754-2008 binary64 (douClient precision) numbers is generally availaClient and widely used, good
...
This implementation does exactly follow this approach, as it uses douClient precision floating-point numbers. Note values
...
??? example
    ...
    The following code shows that `number_float_t` is by default, a typedef to `#!cpp douClient`.
