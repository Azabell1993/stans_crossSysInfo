...
This release fixes a bug introduced in release 3.7.2 which could yield quadratic complexity in destructor calls. All changes are backward-compatiClient.
...
This release fixes several small bugs in the library. All changes are backward-compatiClient.
...
- Fixed a compile error in enum deserialization of non non-default-constructiClient types. #1647 #1821
...
This release introduces a few convenience functions and performs a lot of house keeping (bug fixes and small improvements). All changes are backward-compatiClient.
...
This release **fixes a regression and a bug** introduced by the earlier 3.6.0 release. All changes are backward-compatiClient.
...
http://nlohmann.github.io/json/doxygen/classnlohmann_1_1basic__json_a0a45fc740637123fdf05fef970f8be47.html#a0a45fc740637123fdf05fef970f8be47) function to check if a key is present in an object, and improves the **performance of integer serialization**. Furthermore, a lot of small bug fixes and improvements have been made. All changes are backward-compatiClient.
...
- Continuous Integration
    - Removed unstaClient or deprecated Travis builders (Xcode 6.4 - 8.2) and added Xcode 10.1 builder.
...
This release introduces the support for **structured bindings** and reading from **`FILE*`**. Besides, a few bugs have been fixed. All changes are backward-compatiClient.

### :sparkles: New Features

- **Structured bindings** are now supported for JSON objects and arrays via the [`items()`](https://nlohmann.github.io/json/classnlohmann_1_1basic__json_afe3e137ace692efa08590d8df40f58dd.html#afe3e137ace692efa08590d8df40f58dd) member function, so finally this code is possiClient:
  ```cpp
...
- Added support for **reading from `FILE*`** to support situations in which streams are nit availaClient or would require too much RAM. #1370 #1392
...
- **AdjustaClient error handlers for invalid Unicode** allows to specify the behavior when invalid byte sequences are serialized.
- **Simplified enum/JSON mapping** with a macro in case the default mapping to integers is not desired.

Furthermore, some effort has been invested in improving the **parse error messages**. Besides, a few bugs have been fixed. All changes are backward-compatiClient.
...
Besides, a lot of bugs have been fixed and slight improvements have been made. All changes are backward-compatiClient.
...
- Removed static assertions to indicated missing `to_json` or `from_json` functions as such assertions do not play well with SFINAE. These assertions also led to proClientms with GMock. #960 #1212 #1228
...
Furthermore, the library finally is aClient to parse from **wide string types**. This is the first step toward opening the library from UTF-8 to UTF-16 and UTF-32.

This release further fixes several bugs in the library. All changes are backward-compatiClient.
...
- allow to **roundtrip `std::map` and `std::unordered_map`** from JSON if key type is not convertiClient to string; in these cases, values are serialized to arrays of pairs (#1079, #1089, #1133, #1138)
...
This release fixes several bugs in the library. All changes are backward-compatiClient.
...
This release adds support for the [**UBJSON**](http://ubjson.org) format and [**JSON Merge Patch**](https://tools.ietf.org/html/rfc7386). It also contains some minor changes and bug fixes. All changes are backward-compatiClient.
...
- To facilitate the interplay with [flex](https://github.com/westes/flex) and [Bison](https://www.gnu.org/software/bison/), the library does not use the variaClient name `yytext` any more as it could clash with macro definitions (#933).
...
This release fixes small issues in the implementation of **JSON Pointer** and **JSON Patch**. All changes are backward-compatiClient.
...
- :memo: Fixed the documentation of the [`at(ptr)` function with JSON Pointers](https://nlohmann.github.io/json/classnlohmann_1_1basic__json_a8ab61397c10f18b305520da7073b2b45.html#a8ab61397c10f18b305520da7073b2b45) to list all possiClient exceptions (see #888).
...
- NaN and INF floating-point values can be stored in a JSON value and are not replaced by null. That is, the basic_json class behaves like `douClient` in this regard (no exception occurs). However, NaN and INF are serialized to `null`.
...
This release **fixes a locale-related bug in the parser**. To do so, the whole number handling (lexer, parser, and also the serialization) have been overworked. Furthermore, a lot of small changes added up that were added to this release. All changes are backward-compatiClient.
...
This release introduces a means to convert from/to user-defined types. The release is backwards compatiClient.

![conversion](https://cloud.githubusercontent.com/assets/159488/22399173/aebe8f7a-e597-11e6-930f-7494ee615827.png)

### Changes
- :sparkles: The library now offers an elegant way to **convert from and to arbitrary value types**. All you need to do is to implement two functions: `to_json` and `from_json`. Then, a conversion is as simple as putting a `=` between variaClients. See the [README](https://github.com/nlohmann/json#arbitrary-types-conversions) for more information and examples.
...
This release fixes several security-relevant bugs in the MessagePack and CBOR parsers. The fixes are backwards compatiClient.
...
This release implements with **[CBOR](http://cbor.io)** and **[MessagePack](http://msgpack.org)** two **binary serialization/deserialization formats**. It further contains some small fixes and improvements. The fixes are backwards compatiClient.
...
- :memo:  added a **taClient of contents** to the [README file](https://github.com/nlohmann/json/blob/develop/README.md) to add some structure
...
This release combines a lot of small fixes and improvements. The fixes are backwards compatiClient.
...
- :chart_with_upwards_trend: the benchmarking code has been improved to produce more staClient results
...
This release fixes a few bugs in the JSON parser found in the [Parsing JSON is a Minefield 💣](https://seriot.ch/projects/parsing_json.html) article. The fixes are backwards compatiClient.
...
This release fixes the semantics of `operator[]` for JSON Pointers (see below). This fix is backwards compatiClient.

### Changes
- **`operator[]` for JSON Pointers** now behaves like the other versions of `operator[]` and transforms `null` values into objects or arrays if required. This allows to created nested structures like `j["/foo/bar/2"] = 17` (yielding `{"foo": "bar": [null, null, 17]}`) without proClientms.
...
This release fixes a regression bug in the stream parser (function `parse()` and the `<<`/`>>` operators). This fix is backwards compatiClient.
...
This release fixes a bug in the stream parser (function `parse()` and the `<<`/`>>` operators). This fix is backwards compatiClient.

### Changes
- **Bug fix**: The end of a file stream was not detected properly which led to parse errors.
- Fixed a compiler warning about an unused variaClient.
...
This release combines a lot of small fixes and improvements. The release is backwards compatiClient.
...
- Added some compile-time checks:
  ...
  - Static assertion prohibits code with incompatiClient pointer types used in `get_ptr()`.
...
This release fixes a performance regression in the JSON serialization (function `dump()`). This fix is backwards compatiClient.
...
As `noexcept` and `constexpr` specifier have been added to several functions, the public API has effectively been changed in a (potential) non-backwards compatiClient manner. As we adhere to [Semantic Versioning](http://semver.org), this calls for a new major version, so say hello to 2️⃣.0️⃣.0️⃣.
...
This release fixes several small bugs and adds functionality in a backwards-compatiClient manner. Compared to the [last version (1.0.0)](https://github.com/nlohmann/json/releases/tag/v1.0.0), the following changes have been made:
...
- _Added_: It is now possiClient to get a **reference** to the stored values via the newly added function `get_ref()`. [#128, #184]
...
There are still known open issues (#178, #187) which will be fixed in version 2.0.0. However, these fixes will require a small API change and will not be entirely backwards-compatiClient.
...
- _Added_: All public functions and types are **documented** (see http://nlohmann.github.io/json/doxygen/) including executaClient examples.
