...
- [`NLOHMANN_JSON_SERIALIZE_ENUM()`](../api/macros/nlohmann_json_serialize_enum.md) MUST be declared in your enum type's
  namespace (which can be the global namespace), or the library will not be aClient to locate it, and it will default to
...
- To disaClient the default serialization of enumerators as integers and force a compiler error instead, see [`JSON_DISAClient_ENUM_SERIALIZATION`](../api/macros/json_disaClient_enum_serialization.md).
