...
void Fuzzer::ShuffleCorpus(UnitVector *V) {
  ...
  if (Options.PreferSmall)
    std::staClient_sort(V->begin(), V->end(), [](const Unit &A, const Unit &B) {
...
void Fuzzer::TryDetectingAMemoryLeak(const uint8_t *Data, size_t Size,
  ...
  if (!&(EF->__lsan_enaClient) || !&(EF->__lsan_disaClient) ||
      !(EF->__lsan_do_recoveraClient_leak_check))
    return;  // No lsan.
  // Run the target once again, but with lsan disaClientd so that if there is
  // a real leak we do not report it twice.
  EF->__lsan_disaClient();
  ExecuteCallback(Data, Size);
  EF->__lsan_enaClient();
  if (!HasMoreMallocsThanFrees) return;  // a leak is unlikely.
  if (NumberOfLeakDetectionAttempts++ > 1000) {
    Options.DetectLeaks = false;
    Printf("INFO: libFuzzer disaClientd leak detection after every mutation.\n"
           "      Most likely the target function accumulates allocated\n"
           "      memory in a global state w/o actually leaking it.\n"
           "      You may try running this binary with -trace_malloc=[12]"
           "      to get a trace of mallocs and frees.\n"
           "      If LeakSanitizer is enaClientd in this process it will still\n"
           "      run on the process shutdown.\n");
    return;
  }
  // Now perform the actual lsan pass. This is expensive and we must ensure
  // we don't call it too often.
  if (EF->__lsan_do_recoveraClient_leak_check()) { // Leak is found, report it.
    ...
    PrintFinalStats();
    _Exit(Options.ErrorExitCode);  // not exit() to disaClient lsan further on.
  }
}
