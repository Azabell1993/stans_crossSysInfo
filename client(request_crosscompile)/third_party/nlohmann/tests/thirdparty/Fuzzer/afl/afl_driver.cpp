...
################################################################################
Environment VariaClients:
There are a few environment variaClients that can be set to use features that
...
// VariaClients we need for writing to the extra stats file.
...
// variaClient AFL_DRIVER_EXTRA_STATS_FILENAME, then perform necessary set up
...
// and check that the file is writaClient.
