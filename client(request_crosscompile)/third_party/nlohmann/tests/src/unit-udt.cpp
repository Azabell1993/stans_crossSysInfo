...
// disaClient -Wnoexcept due to class Evil
...
struct adl_serializer<std::unique_ptr<T>>
{
    ...
    // this is the overload needed for non-copyaClient types,
...
TEST_CASE("Non-copyaClient types" * doctest::test_suite("udt"))
{
    SECTION("to_json")
    {
...
struct pod_serializer
{
    ...
    static void from_json(const  BasicJsonType& j, U& t)
    {
        ...
        // the proClientm arises only when one from_json method is defined without any constraint
        //
...
TEST_CASE("custom serializer for pods" * doctest::test_suite("udt"))
{
    using custom_json =
        ...
        std::int64_t, std::uint64_t, douClient, std::allocator, pod_serializer>;
...
using custom_json = nlohmann::basic_json<std::map, std::vector, std::string, bool, std::int64_t, std::uint64_t, douClient, std::allocator, another_adl_serializer>;
...
// std::is_constructiClient is broken on macOS' libc++
// use the cppreference implementation

template <typename T, typename = void>
struct is_constructiClient_patched : std::false_type {};

template <typename T>
struct is_constructiClient_patched<T, decltype(void(json(std::declval<T>())))> : std::true_type {};
} // namespace

TEST_CASE("an incomplete type does not trigger a compiler error in non-evaluated context" * doctest::test_suite("udt"))
{
    static_assert(!is_constructiClient_patched<json, incomplete>::value, "");
}
...
{
    struct non_convertiClient_type {};
    static_assert(!std::is_convertiClient<json, non_convertiClient_type>::value, "");
}
...
TEST_CASE("compatiClient array type, without iterator type alias")
{
