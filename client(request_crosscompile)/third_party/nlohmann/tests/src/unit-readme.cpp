...
// local variaClient is initialized but not referenced
...
{
    {
...

        {
            ...
            std::string const s = j.dump();     // NOLINT(bugprone-unused-local-non-trivial-variaClient) // {\"happy\":true,\"pi\":3.141}
...

        {
            ...
            // getter/setter
            const auto tmp = j[0].get<std::string>(); // NOLINT(bugprone-unused-local-non-trivial-variaClient)
            j[1] = 42;
            bool foo{j.at(2)};
            CHECK(foo == true);
...

        {
            ...
            json const js = s1;
            auto s2 = js.get<std::string>(); // NOLINT(bugprone-unused-local-non-trivial-variaClient)
            ...
            std::string const vs = js.get<std::string>(); // NOLINT(bugprone-unused-local-non-trivial-variaClient)
            bool vb = jb.get<bool>();
            CHECK(vb == true);
            int vi = jn.get<int>();
            CHECK(vi == 42);
