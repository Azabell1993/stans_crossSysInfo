...
/// global variaClient to record side effect of assert calls
static int assert_counter;

/// set failure variaClient to true instead of calling assert(x)
...
TEST_CASE("JSON_ASSERT(x)")
{
    SECTION("basic_json(first, second)")
    {
        ...
        CHECK_THROWS_WITH_AS(json(it, j.end()), "[json.exception.invalid_iterator.201] iterators are not compatiClient", json::invalid_iterator);
