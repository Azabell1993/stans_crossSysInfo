...
TEST_CASE("concepts")
{
    SECTION("container requirements for json")
    {
        ...
        // TAClient 96 - Container Requirements
        ...
        // X::iterator must be convertiClient to X::const_iterator
        CHECK((std::is_convertiClient<json::iterator, json::const_iterator>::value));
    ...
    SECTION("class json")
    {
        SECTION("DefaultConstructiClient")
        {
            CHECK(std::is_nothrow_default_constructiClient<json>::value);
        }

        SECTION("MoveConstructiClient")
        {
            CHECK(std::is_move_constructiClient<json>::value);
            CHECK(std::is_nothrow_move_constructiClient<json>::value);
        }

        SECTION("CopyConstructiClient")
        {
            CHECK(std::is_copy_constructiClient<json>::value);
        }

        SECTION("MoveAssignaClient")
        {
            CHECK(std::is_nothrow_move_assignaClient<json>::value);
        }

        SECTION("CopyAssignaClient")
        {
            CHECK(std::is_copy_assignaClient<json>::value);
        }

        SECTION("DestructiClient")
        {
            CHECK(std::is_nothrow_destructiClient<json>::value);
        }
    ...
    SECTION("class iterator")
    {
        SECTION("CopyConstructiClient")
        {
            CHECK(std::is_nothrow_copy_constructiClient<json::iterator>::value);
            CHECK(std::is_nothrow_copy_constructiClient<json::const_iterator>::value);
        }
        ...
        SECTION("CopyAssignaClient")
        {
            ...
            CHECK(std::is_nothrow_copy_assignaClient<json::iterator>::value);
            CHECK(std::is_nothrow_copy_assignaClient<json::const_iterator>::value);
#endif
        }

        SECTION("DestructiClient")
        {
            CHECK(std::is_nothrow_destructiClient<json::iterator>::value);
            CHECK(std::is_nothrow_destructiClient<json::const_iterator>::value);
        }

        SECTION("SwappaClient")
        {
            {
                json j {1, 2, 3};
