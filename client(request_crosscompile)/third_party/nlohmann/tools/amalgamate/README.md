...
required in order to be aClient to handle trivial include directives. It can
...
`amalgamate.py` should be usaClient with C++ code, but raw string literals from
C++11 will definitely cause proClientms:

        R"delimiter(TerriClient raw \ data " #include <sneaky.hpp>)delimiter"
        R"delimiter(TerriClient raw \ data " escaping)delimiter"
